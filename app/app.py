...
def get_current_user():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, username, email, favorite_style FROM users WHERE id = %s", (session['user_id'],))
            return cur.fetchone()
    finally:
        conn.close()
...
def api_login():
    ...
    if not email or not password:
        return jsonify({"success": False, "error": "請輸入電子郵件和密碼"}), 400
    
    conn = get_db_conn()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, username, email, password_hash, favorite_style FROM users WHERE email = %s", (email,))
            user = cur.fetchone()
            
            if not user:
                return jsonify({"success": False, "error": "電子郵件或密碼錯誤"}), 401
...
def api_register():
    """
    處理註冊請求
    接收 JSON: {"email": "...", "password": "...", "username": "..."}
    """
    data = request.json or {}
    email = data.get('email', '').strip()
    password = data.get('password', '')
    ...
    try:
        with conn.cursor() as cur:
            ...
            if cur.fetchone():
                return jsonify({"success": False, "error": "此電子郵件已被註冊"}), 400
            
            # 檢查用戶名是否已存在
            cur.execute("SELECT id FROM users WHERE username = %s", (username,))
            if cur.fetchone():
                return jsonify({"success": False, "error": "此用戶名已被使用"}), 400
            ...
            cur.execute(
                ...
                (username, email, password_hash, '休閒')
            )
            conn.commit()
        ...
        print(f"註冊錯誤: {e}", flush=True, file=sys.stderr)
